...
**第一次用 GitHub？从 [新手投稿图文教程](新手投稿图文教程.md) 开始。** 另有完整的 [纯文字版](新手投稿纯文字教程.txt)，下载后用记事本即可阅读。只需浏览器、GitHub 账号和程序导出的文件，无需安装 Git 或编写代码。
...
维护者检查文件格式、地图、导入结果、道路连接、版本与作者说明，必要时请投稿者补充游戏内验证。0.5.1 起软件不再设置道路确认状态，旧字段仅为文件兼容保留；实际测试范围写在配置包 README 中，未测试的部分如实写明即可。配置语法通过不代表车辆一定能通行。
